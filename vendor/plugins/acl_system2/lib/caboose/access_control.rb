      ...
      def access_control(actions={})
        ...
        before_filter do |c|
          ...
          if @access.allowed?(c.action_name)
            ...
            # Если не достаточно прав - редирект на главную
            c.send(:redirect_to, '/')
            if c.respond_to?:permission_denied
#              c.send(:permission_denied)
#              flash[:error] = "Доступ ограничен!"
            else  
#              c.send(:render_text, "У Вас не достаточно прав для доступа к #{c.controller_name}/#{c.action_name}!")
#              flash[:error] = "У Вас не достаточно прав для доступа к #{c.controller_name}/#{c.action_name}!"
            end
          end
        end
      end 
